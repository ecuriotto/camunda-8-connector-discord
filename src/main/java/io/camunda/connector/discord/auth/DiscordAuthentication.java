 ...
 * <ul>
 *   <li><b>Bot Token</b> — for full API access (send messages, manage channels, roles)</li>
 *   <li><b>Webhook URL</b> — for simple message sending (no bot required)</li>
 * </ul>
 *
 * <p>Sensitive fields are redacted in {@link #toString()} to prevent secret leakage in logs.
 */
public record DiscordAuthentication(
    ...
    @TemplateProperty(
        group = "authentication",
        label = "Webhook URL",
        description = "Discord webhook URL for sending messages. Use {{secrets.DISCORD_WEBHOOK_URL}}.",
        type = PropertyType.String,
        optional = true)
    String webhookUrl) {
